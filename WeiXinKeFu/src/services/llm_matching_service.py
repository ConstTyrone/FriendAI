...
## 判断要求
1. 采用**积极匹配**的思维，重点关注联系人的潜力和适配性
2. 深入理解意图的真实需求，包括显性和隐性要求
3. 全面评估联系人的能力、经验和发展潜力
4. 考虑行业背景、转岗可能性和实际合作价值
5. 注意否定条件（"不要"、"除了"等）的处理
6. 给出0-1之间的匹配分数：
   - 0.8-1.0: 高度匹配，强烈推荐
   - 0.6-0.8: 良好匹配，值得考虑
   - 0.4-0.6: 中等匹配，有潜力但需进一步评估
   - 0.2-0.4: 弱匹配，部分符合但存在明显差距
   - 0.0-0.2: 基本不匹配
7. 提供你的判断置信度（0-1）
...
注意：
- match_score >= 0.5 时，is_match 应为 true（降低门槛，更多机会）
- 重点关注联系人的**优势和潜力**，而不是缺陷
- 对于转岗、跨行业的可能性要保持开放态度
- 解释要具体、有说服力，突出匹配的亮点
- 建议要实用、可操作，帮助促成合作"""
